...
export const RepliesPage = () => {
    ...
    const fetchTweets = async () => {
        try {
            const accessToken = localStorage.getItem('access_token');
            const response = await axios.get(`${apiUrl}/tweets/get_curr_user_comments`, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`
                },
            ...
            if (Array.isArray(response.data.comments)) {
                const sortedTweets = response.data.comments.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
                ...
                <Col xs={{span:9, offset:2}}>
                    ...
                    <Container fluid>
                        <Row>
                            ...
                            <Col>
                                <Button style={{textDecoration:"underline", textDecorationThickness:"4px", color:"black", textDecorationColor:"blue"}} onClick={()=>navigate("/replies")}>Replies</Button>
                            </Col>
                            <Col>
